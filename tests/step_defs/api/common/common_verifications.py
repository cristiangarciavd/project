...
def step_verify_status(request, status_code):
...
def step_verify_variable_update(request, variable, value):
...
def step_verify_body_update(request, datatable, body):
...
@then(parsers.parse('the response schema should be verified with "{json_template}"'))  # noqa:E501
def step_verify_response_schema(json_template, request):  # noqa:E501 pylint: disable=W0613
